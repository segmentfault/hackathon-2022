				...
				<table border="1"  >
					...
					<c:forEach var="item" varStatus="i" items="${list}"> 
						<tr>
							...
							<td>
								<c:if test="${item.verify=='0'}">未审核</c:if>
								<c:if test="${item.verify=='1'}">通过</c:if>
								<c:if test="${item.verify=='2'}">未通过</c:if>
							</td>
								
							<td>
								 	<c:if test="${item.verify=='0'||item.verify=='2'}"><a
										href="${base }/servlet/ProjectServlet?method=updatePage&id=${item.id}&flag=1">修改</a>|
									<a
										href="${base }/servlet/ProjectServlet?method=deleteProject&id=${item.id}">删除</a></c:if>
								<c:if test="${item.verify=='0'||item.verify=='1'}">
								<a
