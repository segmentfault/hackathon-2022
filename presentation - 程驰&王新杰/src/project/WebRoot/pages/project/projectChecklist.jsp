				...
				<table border="1"  >
					...
					<c:forEach var="item" varStatus="i" items="${list}"> 
						<tr>
							...
							<td>
								 	<c:if test="${item.checkdate==null}"> <a
										href="${base }/servlet/ProjectServlet?method=updatePage&id=${item.id}&flag=4">添加验收评价</a> </c:if><c:if test="${item.checkdate!=null}"> <a
										href="${base }/servlet/ProjectServlet?method=updatePage&id=${item.id}&flag=5">查看</a> </c:if>
							</td>
						</tr>
					</c:forEach>
				</table>
