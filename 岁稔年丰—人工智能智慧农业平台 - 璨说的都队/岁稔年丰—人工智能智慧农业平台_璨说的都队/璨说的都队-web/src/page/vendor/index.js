...
export default class Index extends React.Component{
    ...
    handleClick(item){
        NavigationUtil.goto(RouteConfig.Vendormf,this.props.history)({state:{item}})
    }
    constructor(props) {
        super(props);
        this.columns=[
            {
                ...
                width:'10%',
                fixed:'left',
                ...this.getColumnSearchProps('tVendId')
            },
            {
                title:'商贩姓名',
                dataIndex:'tVendName',
                width:'20%',
                fixed:'left',
                ...this.getColumnSearchProps('tVendName')
            },
            {
                title:'商贩地址',
                dataIndex:'tVendAddress',
                ...this.getColumnSearchProps('tVendAddress')
            },
            {
                title:'商贩手机号',
                dataIndex:'tVendTele',
                width:'20%',
                ...this.getColumnSearchProps('tVendTele')
            },
            {
                title:'删除',
                dataIndex: 'operation',
    ...
    loadData=pageIndex=>{
        ...
        api.vendorList({pageIndex,pageSize:PAGE_SIZE})
            .then(res=>res.json())
            .then(result=>{
    ...
    removecraft = record => {
        api.removeVendor(record.tVendId)
            .then(res=>res.json())
            .then(result=>{
