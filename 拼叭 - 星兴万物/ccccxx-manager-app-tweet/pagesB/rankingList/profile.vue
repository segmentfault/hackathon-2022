		...
		<view class="activity-image" v-if="userInfo">
			<swiper class="mineBanner_swiper" :indicator-dots="indicatorDots" :autoplay="autoplay" :interval="interval"
				...
				<swiper-item v-for="(item,index) in userInfo.pictures" :key='index'>
					<image style="height: 100%; width: 100%" :src="item.url" mode="aspectFill"
						@click='preview(item.url)'>
				</swiper-item>
			</swiper>
		</view>
		<view class="page-ctn" v-if="!isMe">
			<view class="fixed" style="z-index:2;" v-if="boostId">
				<view v-if="boost=='false'" class="boost"><button style="background-color: #000;" @click="setBoost()">给他投票</button>
				</view>
				<view v-else class="boost"><button style="background-color: #999999;z-index: 2;">已投票</button></view>
			</view>
		</view>
		...
		<view class="page-ctn">
			<view class="card-bg">
				<view class="man-item-l">
					<image class="header"
						:src="userInfo.head||'https://basic-1258404477.cos.ap-nanjing.myqcloud.com/mp-static/icon-pinbar2.png'"
						v-if="userInfo.head">
					</image>
					<template v-else-if="isMe">
						<button v-if="canIUseGetUserProfile" class='user_button' plain='true' @click="getUserInfo">
							<image
								src="https://basic-1258404477.cos.ap-nanjing.myqcloud.com/mp-static/icon-pinbar2.png">
						</button>
					...
					<image v-else class="header"
						src='https://basic-1258404477.cos.ap-nanjing.myqcloud.com/mp-static/icon-pinbar2.png'></image>
				...
				<view class="man-item-r">
					...
					<view class="flex-s" style="margin-top: 9upx;">
						<view class="flex-c interest-line">
							兴趣：<view class="interest" v-for="(item,index) in userInfo.interests" :key="index">{{item}}
							</view>
						</view>
					</view>
		...
		<view class="game-ctn" v-if="!isMe&&coupon.length">
			...
			<view class="list">
				<view class="list-item" v-for="(item,i) in coupon" :key="i">
					<view class="list-item-left">{{item.discount}}<text v-if="item.type === 1">折</text><text
							v-else>元</text></view>
			...
			showModal(item) {
				...
				setTimeout(() => {
					drawQrcode({
						...
						height: 180,
						canvasId: "qrcode",
						text: "https://pinbar.vip/h5/supplier/home/?consumeCode=" + item.usedCode,
					});
					this.showCode = true;
				}, 200);
			},
