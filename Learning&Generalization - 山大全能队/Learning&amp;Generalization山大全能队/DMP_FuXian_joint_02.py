...
if __name__ == "__main__":
    ...
    data = np.loadtxt("GMM_GMR_joint1.txt", delimiter=' ')
    #取出所有x的值
    data1 = data[1,:].T 
    data = np.loadtxt("GMM_GMR_joint2.txt", delimiter=' ')
    #取出所有x的值
    data2 = data[1,:].T 
    data = np.loadtxt("GMM_GMR_joint3.txt", delimiter=' ')
    #取出所有x的值
    data3 = data[1,:].T 
    data = np.loadtxt("GMM_GMR_joint4.txt", delimiter=' ')
    #取出所有x的值
    data4 = data[1,:].T 
    data = np.loadtxt("GMM_GMR_joint5.txt", delimiter=' ')
    #取出所有x的值
    data5 = data[1,:].T 
    data = np.loadtxt("GMM_GMR_joint6.txt", delimiter=' ')
    #取出所有x的值
    data6 = data[1,:].T 
    data_len = 119
    y_demo = np.zeros((6, data_len))
    # print(y_demo)
    y_demo[0,:] = np.array(data1)
    y_demo[1,:] = np.array(data2)
    y_demo[2,:] = np.array(data3)
    y_demo[3,:] = np.array(data4)
    y_demo[4,:] = np.array(data5)
    y_demo[5,:] = np.array(data6)
    # DMP learning
    ...
    # data1 = pd.DataFrame(y_reproduce[:,0])
    # data2 = pd.DataFrame(y_reproduce[:,1])
    # data3 = pd.DataFrame(y_reproduce[:,2])
    # data4 = pd.DataFrame(y_reproduce[:,3])
    # data5 = pd.DataFrame(y_reproduce[:,4])
    # data6 = pd.DataFrame(y_reproduce[:,5])
    #打印速度
    data1 = pd.DataFrame(dy_reproduce[:,0])
    data2 = pd.DataFrame(dy_reproduce[:,1])
    data3 = pd.DataFrame(dy_reproduce[:,2])
    data4 = pd.DataFrame(dy_reproduce[:,3])
    data5 = pd.DataFrame(dy_reproduce[:,4])
    data6 = pd.DataFrame(dy_reproduce[:,5])

    

    # writer = pd.ExcelWriter('C:/Users/admin/Desktop/数据/DMP_FuXian_joint.xlsx')		# 写入Excel文件
    # data1.to_excel(writer, 'Sheet1', float_format='%.5f')		# ‘page_1’是写入excel的sheet名
    # data2.to_excel(writer, 'Sheet2', float_format='%.5f')	
    # data3.to_excel(writer, 'Sheet3', float_format='%.5f')
    # data4.to_excel(writer, 'Sheet4', float_format='%.5f')	
    # data5.to_excel(writer, 'Sheet5', float_format='%.5f')	
    # data6.to_excel(writer, 'Sheet6', float_format='%.5f')		
    # index=None
    # header=None
    # writer.save()
    # writer.close()
    # set new initial and goal poisitions
    # y_reproduce_2, dy_reproduce_2, ddy_reproduce_2 = dmp.reproduce(tau=0.5, initial=[2, 1, 2], goal=[-3, 2, 1])
    # y_reproduce_2, dy_reproduce_2, ddy_reproduce_2 = dmp.reproduce(tau=0.5, initial=[0.2, 0.3, 0.8], goal=[0.8, -0.6, 0.2])
    # y_reproduce_2, dy_reproduce_2, ddy_reproduce_2 = dmp.reproduce(tau=0.5, initial=[0.2, 0.3, 0.8], goal=[0.085958131,	-0.66645278, 0.392605114])
    # y_reproduce_2, dy_reproduce_2, ddy_reproduce_2 = dmp.reproduce(tau=0.5, initial=[0.105303239, -0.421648794, 0.788584089], goal=[0.8, -0.6, 0.2])
    # y_reproduce_2, dy_reproduce_2, ddy_reproduce_2 = dmp.reproduce(tau=0.5, initial=[-0.0005, -0.2, 0.9], goal=[0.045771087, -0.541749587, 0.402265463])
    # fig=plt.figure()
    # ax1 = Axes3D(fig)
    ...
    # ax1.plot3D(y_demo[0,:],y_demo[1,:],y_demo[2,:],'g')    #绘制空间曲线
    # ax1.plot3D(y_reproduce[:,0],y_reproduce[:,1],y_reproduce[:,2],'b') 
    # ax1.set_xlabel('X position')
    # ax1.set_ylabel('Y position')
    # ax1.set_zlabel('Z position')
    # ax1.plot3D(y_reproduce_2[:,0],y_reproduce_2[:,1],y_reproduce_2[:,2],'c') 
    # plt.show()
    plt.figure(figsize=(10, 5))
    plt.plot(y_demo[0,:], 'deeppink', label='demo1')
    plt.plot(y_reproduce[:,0], 'r--', label='reproduce1')
    # plt.plot(y_reproduce_2[:,0], 'r-.', label='reproduce12')
    plt.plot(y_demo[1,:], 'crimson', label='demo2')
    plt.plot(y_reproduce[:,1], 'g--', label='reproduce2')
    # plt.plot(y_reproduce_2[:,1], 'm-.', label='reproduce22')
    plt.plot(y_demo[2,:], 'darkviolet', label='demo3')
    plt.plot(y_reproduce[:,2], 'b--', label='reproduce3')
    # plt.plot(y_reproduce_2[:,2], 'y-.', label='reproduce32')
    plt.plot(y_demo[3,:], 'teal', label='demo4')
    plt.plot(y_reproduce[:,3], 'c--', label='reproduce4')
    # plt.plot(y_reproduce_2[:,0], 'r-.', label='reproduce12')
    plt.plot(y_demo[4,:], 'yellow', label='demo5')
    plt.plot(y_reproduce[:,4], 'y--', label='reproduce5')
    # plt.plot(y_reproduce_2[:,1], 'm-.', label='reproduce22')
    plt.plot(y_demo[5,:], 'lime', label='demo6')
    plt.plot(y_reproduce[:,5], 'k--', label='reproduce6')
    # plt.plot(y_reproduce_2[:,2], 'y-.', label='reproduce32')
    ...
    plt.ylabel('joint', fontproperties = 'Times New Roman',fontsize=35)
    plt.show()
