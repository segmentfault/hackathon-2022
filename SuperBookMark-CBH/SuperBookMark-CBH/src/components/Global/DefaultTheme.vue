<template>
  ...
  >
    ...
    <el-alert :description="$t('themeWarningText')" type="info" show-icon :closable="false" />
    <template #footer>
...
export default defineComponent({
  ...
  setup() {
    ...
    const themeList: any = window.innerWidth < 500 ? [...themeList1] : [...themeList1]

    const activeTheme = ref()
    const submit = () => {
      const theme = themeList.find((item: any) => item.label === 'Multiple')
      if (theme && theme.json) {
        const {
          list,
          affix,
