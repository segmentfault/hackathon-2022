...
if __name__ == "__main__":
    ...
    data = np.loadtxt("GMM_GMR_X.txt", delimiter=' ')
    #取出所有x的值
    dataX = data[1,:].T 
    data = np.loadtxt("GMM_GMR_Y.txt", delimiter=' ')
    #取出所有y的值
    dataY = data[1,:].T 
    data = np.loadtxt("GMM_GMR_Z.txt", delimiter=' ')
    #取出所有z的值
    dataZ = data[1,:].T 
    data_len = 200
    y_demo = np.zeros((3, data_len))
    # print(y_demo)
    y_demo[0,:] = np.array(dataX)
    y_demo[1,:] = np.array(dataY)
    y_demo[2,:] = np.array(dataZ)
    # DMP learning
    ...
    writer = pd.ExcelWriter('C:/Users/admin/Desktop/数据/DMP_FuXian_XYZ.xlsx')		# 写入Excel文件
    ...
    plt.plot(y_demo[0,:], 'g', label='demoX')
    plt.plot(y_reproduce[:,0], 'r--', label='reproduceX')
    # plt.plot(y_reproduce_2[:,0], 'r-.', label='reproduce12')
    plt.plot(y_demo[1,:], 'b', label='demoY')
    plt.plot(y_reproduce[:,1], 'm--', label='reproduceY')
    # plt.plot(y_reproduce_2[:,1], 'm-.', label='reproduce22')
    plt.plot(y_demo[2,:], 'c', label='demoZ')
    plt.plot(y_reproduce[:,2], 'y--', label='reproduceZ')
