<template>
	<view class="wrap">
		<swiper class="swiper" :style="{height: height *2 + 'rpx'}" :autoplay="autoplay" :interval="interval" :duration="duration"
		 :circular='loop'  @animationfinish="change" :previous-margin='previousMargin + "rpx"' :next-margin='nextMargin + "rpx"'>
			<swiper-item v-for="(item,index) in list" :key='index' @click="$emit('clickItem',item)">
				<view v-if="slots" class="item" :class="[!crown ? '' : current==index ? 'crown-active':'crown']">
					<image class="item-img" :class="[imgShadow?'imgShadow':'']" :src="item[imgKey] || item" :style="{ borderRadius: imgRadius + 'px',width:imgWidth}"
					 mode=""></image>
				</view>
			    <slot :data="item"></slot>
			</swiper-item>
