<template>
  <view
    class="cmd-cell-item"
    :class="{ 'is-disabled': disabled, 'no-border': !border }"
    @tap="$_click"
  ...
  >
    <view class="cmd-cell-item-body">
      ...
      <view class="cmd-cell-item-right" :style="{ background: bg }" v-if="slotRight">
        <slot></slot>
        <view class="cmd-cell-icon-arrow-right" v-if="arrow">
          <u-icon
            class="icon"
            name="arrow-right"
            color="#999999"
      ...
      <view class="cmd-cell-item-right" v-if="!slotRight">
        <text :class="addon.length > 18 ? 'cmd-cell-addon-text' : ''">{{
          addon
        }}</text>
