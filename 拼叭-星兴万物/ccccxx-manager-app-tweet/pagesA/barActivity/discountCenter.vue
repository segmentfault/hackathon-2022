<template>
  <view class="activity-discount-center">
    ...
    <subscribe
      ...
      :src="picture"
    />
    <!-- src="https://image-1306191496.cos.ap-nanjing.myqcloud.com/dca5a179-cf5e-4b90-85b7-e2a0e98eea85.png" -->
    <!-- 好友助力-->
    <haveSubscribedTo
...
export default {
  data() {
    return {
      ...
      share: false,
      statusArr: [2, 3, 4, 5, 1],
      activeStatus: 1, // 1个人助力成功页面前端随便定的值 2助力中 3助力完成 4已取消报名 || 未报名  5已核销 1是表示自己已经助力过了
    }
  },
  onLoad(params = {}) {
  ...
  methods: {
    ...
    async queryBoost() {
      ...
      gameService
        .boost(this.shareUserId, 1, 4, this.query.activityId)
        .then(() => {
          this.activeStatus = 1
        })
    },
    ...
    getShareActivityDetail() {
      gameService
        .getShareActivityDetail(this.shareUserId, this.query.activityId)
        .then((activeDetail) => {
          const newBoostUser = []
          const boostUsers = []
    ...
    queryActivityDetail() {
      gameService
        .applyActivityDetail(this.query.activityId)
        .then((activeDetail) => {
          const newBoostUser = []
          const boostUsers = []
          activeDetail.boostUsers = activeDetail.boostUsers || []
  ...
  onShareAppMessage(e) {
    ...
    // 自己邀请出去助力
    let path = `/pagesA/barActivity/discountCenter?shareUserId=${this.myUserId}&activityId=${this.query.activityId}&isHome=1`
    // 朋友邀请出去助力
    if (this.shareUserId) {
      path = `/pagesA/barActivity/discountCenter?shareUserId=${this.shareUserId}&activityId=${this.query.activityId}&isHome=1`
    }
    return {
