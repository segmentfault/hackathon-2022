<template>
  <view style="position: relative" :style="{background:bg}">
    <view
      class="head-custom"
      v-if="custom"
      :style="{
    ...
    >
      <u-icon
        v-if="!hideBack"
        class="back-icon"
        :style="{ paddingTop: titleBarHeight + statusBarHeight + 'rpx' }"
        @click="back"
        name="arrow-left"
        :color="changeColor ? (Number(ratio) >= 0.5 ? '#333' : '#FFF') : color"
        size="36"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      headHeight:
        (titleBarHeight + uni.getSystemInfoSync().statusBarHeight) / 2,
      ratio: 0,
      headCustomStyle: "",
      scrollTop: 0,
    };
