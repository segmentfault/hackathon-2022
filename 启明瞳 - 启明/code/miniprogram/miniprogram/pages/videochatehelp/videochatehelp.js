...
const app = getApp();
const Utils = require('../../utils/agorautil.js');
const AgoraMiniappSDK = require("../../libs/mini-app-sdk-production.js");
const max_user = 10;
...
Page({
  ...
  onLoad: function (options) {
    ...
    //发送志愿者请求
    this.askForHekp(options.channel);
  },

  /**
   * 开启会议室直播内容
   */
  askForHekp: function (channel) {
  ...
  initLayouter: function () {
    ...
    const systemInfo = wx.getStorageSync('sysInfo');
    this.setData({
      screen_width: systemInfo.windowWidth,
      screen_height: systemInfo.windowHeight - 64,
    })
   ...
   */
  onUnload: function () {
    ace.showToast('您正在退出房间...请点击中间弹框确认退出帮助！');
    wx.showModal({
      title: '确认退出',
      content: '是否退出帮助',
      success(res) {
